...
CREATE TABLE IF NOT EXISTS `database`.`Repositorio` (
  ...
  `acceso_repositorio` ENUM('RESTRINGIDO', 'ORGANIZACION', 'CUALQUIER_PERSONA_CON_EL_ENLACE') NOT NULL,
...
CREATE TABLE IF NOT EXISTS `database`.`Version_API` (
  ...
  UNIQUE INDEX `numero_version_UNIQUE` (`numero_version` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `database`.`Usuario` (
  ...
  `sexo_usuario` ENUM('HOMBRE', 'MUJER') NULL,
...
CREATE TABLE IF NOT EXISTS `database`.`Solicitud_acceso_API` (
  ...
  `usuario_usuario_id` INT UNSIGNED NOT NULL,
  `api_api_id` INT UNSIGNED NOT NULL,
  `usuario_usuario_id1` INT UNSIGNED NOT NULL,
  ...
  INDEX `fk_solicitud_acceso_API_usuario1_idx` (`usuario_usuario_id1` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_solicitud_acceso_API_api1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_solicitud_acceso_API_usuario1`
    FOREIGN KEY (`usuario_usuario_id1`)
    REFERENCES `database`.`Usuario` (`usuario_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `database`.`Credencial_API` (
  `credencial_id` INT UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `database`.`Impersonacion` (
  `idImpersonacion` INT UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `database`.`Payload` (
  ...
  `Mensaje_mensaje_id` INT NOT NULL,
